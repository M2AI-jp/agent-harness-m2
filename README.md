...
Less approval, not more autonomy: M2 は、人間の確認回数を減らすために、AI の作業前ループを制御します。
...
M2 does not commit, push, open PRs, merge, or sync local main as part of core behavior.

GitHub workflow は M2 core の外側です。`/map` は候補状態を検知・推奨しますが、commit / push / PR / merge / local sync は実行しません。
